...
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/kill_hanul_process.sh"
...
CMD_EMPTY="exec bash"
CMD_WEBOTS="deactivate 2>/dev/null; $SETUP_CMD; export PYTHONPATH=$PROJECT_ROOT; webots $WEBOTS_WORLD > /tmp/hanul_webots.log 2>&1 & disown; echo 'Webots started in background. log: /tmp/hanul_webots.log'; exec bash"
