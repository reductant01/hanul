...
CMD_TELEOP="$SETUP_CMD; cd $PROJECT_ROOT && PYTHONPATH=$PROJECT_ROOT:\$PYTHONPATH python3 common/teleop_keyboard_hold.py --ros-args -p speed:=$TELEOP_SPEED -p turn:=$TELEOP_TURN; exec bash"
