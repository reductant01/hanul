...
tmux split-window -h -t "${SESSION}:0.0"
  tmux send-keys -t "${SESSION}:0.1" "cd $PROJECT_ROOT && source /opt/ros/jazzy/setup.bash && [ -f ~/ros2_ws/install/setup.bash ] && source ~/ros2_ws/install/setup.bash ; export PYTHONPATH=$PROJECT_ROOT:\$PYTHONPATH ; LIDAR_PORT=$LIDAR_PORT && [ -e \"\$LIDAR_PORT\" ] && sudo chmod 666 \"\$LIDAR_PORT\" ; ros2 run sllidar_ros2 sllidar_node --ros-args -p channel_type:=serial -p serial_port:=\$LIDAR_PORT -p serial_baudrate:=1000000 -p frame_id:=lidar_link -p inverted:=false -p angle_compensate:=true -p scan_mode:=Standard -r scan:=/scan_raw & python3 common/lidar_scan_mask.py --ros-args --params-file $PROJECT_ROOT/config/hanul/laser_mask.yaml" C-m
