...
tmux send-keys -t "${SESSION}:0.1" "cd $PROJECT_ROOT && source /opt/ros/jazzy/setup.bash && [ -f ~/ros2_ws/install/setup.bash ] && source ~/ros2_ws/install/setup.bash ; export PYTHONPATH=$PROJECT_ROOT:\$PYTHONPATH ; LIDAR_PORT=$LIDAR_PORT && [ -e \"\$LIDAR_PORT\" ] && sudo chmod 666 \"\$LIDAR_PORT\" ; ros2 launch sllidar_ros2 sllidar_s2_launch.py serial_port:=\$LIDAR_PORT --ros-args -r scan:=/scan_raw & python3 common/lidar_scan_mask.py --ros-args --params-file $PROJECT_ROOT/config/hanul/laser_rear_mask.yaml" C-m
