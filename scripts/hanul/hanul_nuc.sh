...
MODE_INPUT="${1:-loc}"
case "$MODE_INPUT" in
  map|mapping)   MODE="robot_map" ;;
  loc|localization) MODE="robot_loc" ;;
  *)
    echo "사용법: $0 [map|loc]"
    echo "  map: 실제 로봇 맵핑 (컨트롤러 + SLAM + RViz + Teleop). 라이다 A1은 별도 터미널에서 /scan 발행 필요."
    echo "  loc: 저장된 맵 기준 로컬/네비 (컨트롤러 + Map Server + AMCL + Nav2)"
    exit 1
    ;;
esac

if [[ "$MODE" == "robot_map" ]]; then
