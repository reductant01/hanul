...
CMD_TELEOP="$SETUP_CMD; ros2 run teleop_twist_keyboard teleop_twist_keyboard; exec bash"
...
if [[ "$MODE" == "loc" ]]; then
  ...
  CMD_TOP_3="$SETUP_CMD; ros2 run nav2_amcl amcl --ros-args -p use_sim_time:=false -p scan_topic:=/scan -p base_frame_id:=base_footprint -p odom_frame_id:=odom -p global_frame_id:=map; exec bash"
