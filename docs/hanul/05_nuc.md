...
**이유:** udev가 USB 장치에 부여하는 번호가 연결 순서 등에 따라 달라짐.
...
**이유:** 다이나믹셀이 Position 모드(3)라 Goal Velocity를 무시함.

**수정:** `hanul_hardware_nuc.py`에서 초기화 시 Operating Mode = 1(Velocity) 설정. Dynamixel Wizard에서도 ID 1·2·3 모두 Velocity로 설정 후 EEPROM 저장·재부팅.
...
**이유:** ID 2·3이 여전히 Position 모드로 설정되어 있음.

**수정:** Dynamixel Wizard에서 ID 2, ID 3도 Operating Mode = Velocity(1)로 바꾸고 EEPROM 저장, USB 재연결 또는 재부팅.
...
**이유:** (1) 오도메트리가 "이전 엔코더 = 0"으로 두어 첫 프레임에서 (현재 틱 − 0)×R 같은 큰 델타가 나옴. (2) 엔코더 값이 틱(0~4095)인데 라디안으로 변환하지 않아 스케일이 틀림.
...
**이유:** 로봇/모터 배선과 코드 좌표계가 반대임.

**수정:** `common/ros_bridge.py`의 cmd_vel_callback에서 `vx=-msg.linear.x`, `vy=msg.linear.y`, `w=-msg.angular.z` 로 부호 반전.
...
**이유:** angular 속도에 대한 상한이 없거나 linear에 비해 비율이 큼.
...
**이유:** 가속 완화 때문에 목표 0에도 점점 0으로 수렴하던 동작을 함.

**수정:** `common/omni_velocity.py`에서 목표가 모두 0일 때는 현재 속도를 0으로 바로 설정하고 return 하도록 변경. 이동 중 가속 완화는 유지.

---

## 8. RViz "Message Filter dropping message" (lidar_link 타임스탬프) / Local Costmap 오류

**오류:** RViz에서 "Message Filter dropping message: frame 'lidar_link' at time ... the timestamp on the message is earlier than all the data in the transform cache" 경고가 반복되고, Local Costmap 디스플레이에 주황색 느낌표 오류가 뜸.

**이유:** 스캔 메시지와 TF 타임스탬프가 맞지 않아 메시지가 버려짐.

**수정:** (1) `config/hanul/rviz_map.rviz`, `config/hanul/rviz_loc.rviz`에서 LaserScan 디스플레이의 **Tolerance를 10.0**으로 설정. (2) `config/hanul/nav2_params.yaml`의 local_costmap → voxel_layer에 **tf_filter_tolerance: 10.0** 추가. (3) global_costmap → obstacle_layer에도 **tf_filter_tolerance: 10.0** 추가.
